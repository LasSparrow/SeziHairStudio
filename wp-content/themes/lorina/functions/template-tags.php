...
if ( ! function_exists( 'lorina_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function lorina_posted_on() {
...
if ( ! function_exists( 'lorina_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function lorina_posted_by() {
...
if ( ! function_exists( 'lorina_entry_cats' ) ) :
	/**
	 * Prints HTML with meta information for the categories only.
	 */
	function lorina_entry_cats() {
		// Hide category text for pages.
		if ( 'post' === get_post_type() ) {

			/* translators: used between list items, there is a space after the comma */
			$list_item_seperator = esc_html__( ', ', 'lorina' );

			$categories_list = get_the_category_list( $list_item_seperator );
			if ( $categories_list ) {
				echo '<span class="cat-links"><i class="lorina-icon-folder"></i> ' . $categories_list . '</span>';
			}
...
if ( ! function_exists( 'lorina_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function lorina_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $author_post_count > 1 ) {
				...
				$author_post_count_output = sprintf( ' - <span class="author-post-count"><a href="%1s">' . esc_html__( '%2s posts', 'lorina' ) . '</a></span>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), $author_post_count );
			} else {
			...
			if ( $tags_list ) {
				echo '<span class="tags-links"><i class="lorina-icon-tag"></i> ' . $tags_list . '</span>';
			}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'lorina' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'lorina' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'lorina_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function lorina_post_thumbnail() {
...
if ( ! function_exists( 'lorina_related_post_thumbnail' ) ) :
	function lorina_related_post_thumbnail($related_id) {
...
if ( ! function_exists( 'lorina_single_excerpt' ) ) :
	/**
	 ...
	 *
	 */
	function lorina_single_excerpt() {
