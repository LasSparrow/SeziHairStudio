...
if ( count( $related_posts ) > 0 ) { 
	...
	<div class="related-posts">
		<h3><?php esc_html_e( 'Related', 'lorina' ) ;?></h3>
			...
			<article id="post-<?php echo $related_id; ?>" <?php post_class('', $related_id); ?>>
				...
				if ( get_post_format($related_id) == 'video' ) {
					...
					if ( ! empty( $video ) ) {
						...
						lorina_related_post_thumbnail($related_id);
					}
				} else {
					lorina_related_post_thumbnail($related_id);
				}
				...
				<header class="entry-header">
					...
					?>
						<h3 class="entry-title"><a href="<?php echo esc_url( get_permalink($related_id) ); ?>" rel="bookmark"><?php esc_html_e( 'No Title', 'lorina' ); ?></a></h3>
					<?php
					} else {
				...
				<div class="entry-content">
					...
					if ( 'post' === get_post_type($related_id)) : ?>
						<a class="more-tag button" href="<?php echo esc_url( get_the_permalink($related_id) ); ?>" title="<?php echo esc_attr( get_the_title($related_id) ); ?>"><?php esc_html_e( 'Continue Reading', 'lorina' ); ?></a>
					<?php endif; ?>
				</div><!-- .entry-content -->
